...
const ORB_TRANSITION_SECS   = 5.0;    // how long orb color/size transition takes (seconds)
...
function initBackground() {
    ...
    const orbs = BASE_CONFIGS.map((cfg, i) => {
        ...
        // Stamp the transition duration from the config constant
        // so it's always in sync with ORB_TRANSITION_SECS
        el.style.transition = [
            `opacity 3s ease`,
            `background ${ORB_TRANSITION_SECS}s ease-in-out`,
    ...
    function tick(now) {
        ...
        orbs.forEach(state => {
            ...
            // Keep sizePx in sync without fighting the CSS transition
...
// ============================================================
//  ORB SKIN UPDATE — respects ORB_TRANSITION_SECS
// ============================================================
...
function updateActiveOrbSkins() {
    ...
    states.forEach(state => {
        ...
        // Re-stamp transition so it always matches ORB_TRANSITION_SECS
        // even if the element was created before the constant was read
        ...
        // Setting these triggers the CSS transition smoothly
