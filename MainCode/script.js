...
function initBackground() {
    ...
    const orbs = BASE_CONFIGS.map((cfg, i) => {
        ...
        const state = { 
            el, sizePercent: cfg.sizePercent, sizePx: 0, blurBase: cfg.blurBase, 
            isSecondary: cfg.isSecondary, opacity: cfg.opacity, x: startX, y: startY, 
            tx: 0, ty: 0, ox: startX, oy: startY, duration: 1, elapsed: 0, 
            speedMultiplier: 1.0 
        };
    ...
    function tick(now) {
        ...
        orbs.forEach(state => {
            ...
            if (!isNaN(currentComputedSize)) { state.sizePx = currentComputedSize; }
...
function updateActiveOrbSkins() {
    ...
    states.forEach((state) => {
        ...
        // Transitions smoothly apply updates via CSS
